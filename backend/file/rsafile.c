...
void encrypt_file(const char *input_filename, const char *output_filename, RSA *rsa) {
    ...
    int encrypted_length = RSA_public_encrypt(input_file_len, plaintext, encrypted, rsa, PADDING);
...
void decrypt_file(const char *input_filename, const char *output_filename, RSA *rsa) {
    ...
    unsigned char *encrypted = (unsigned char *)malloc(input_file_len);
    if (!encrypted) {
        perror("Failed to allocate memory");
        exit(EXIT_FAILURE);
    }
    fread(encrypted, 1, input_file_len, input_file);
    fclose(input_file);
    ...
    int decrypted_length = RSA_private_decrypt(input_file_len, encrypted, decrypted, rsa, PADDING);
